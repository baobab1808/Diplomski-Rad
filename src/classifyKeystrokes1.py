...
class ClassifyKeystrokes:
    def __init__(self, files, outfile, shuffle=True):
        ...
        model2 = "..\\out\\raw_sentence\\models\\fc_nn_model_97_white.txt"
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for data in self.validloader:
                # extract the images (input data) and labels (true labels) from the data
                images, labels = data
    ...
    def validate_sentence(self, path):
        ...
        print("")
        # return accuracy, predicted sentence and char_predict dictionary
        return acc, sentence, char_predict
...
def main():
    ...
    files = []
    # state the source directory where all the files with data are stored
    source_dir = Path('..\\out\\keystrokes\\keyboards\\w\\')
    # create an outfile
